...
const _memory = require("@llamaindex/core/memory");
...
var _initProto;
class CondenseQuestionChatEngine extends _chatengine.BaseChatEngine {
    static{
        ({ e: [_initProto] } = _apply_decs_2203_r(this, [
            [
                _decorator.wrapEventCaller,
                2,
                "chat"
            ]
        ], []));
    ...
    constructor(init){
        ...
        this.memory = new _memory.ChatMemoryBuffer({
            chatHistory: init?.chatHistory
        });
        this.llm = _Settings.Settings.llm;
        this.condenseMessagePrompt = init?.condenseMessagePrompt ?? _prompts.defaultCondenseQuestionPrompt;
    }
    _getPromptModules() {
        return {};
    }
    ...
    async condenseQuestion(chatHistory, question) {
        const chatHistoryStr = (0, _utils.messagesToHistory)(await chatHistory.getMessages());
    ...
    async chat(params) {
        ...
        const chatHistory = params.chatHistory ? new _memory.ChatMemoryBuffer({
            chatHistory: params.chatHistory instanceof _memory.BaseMemory ? await params.chatHistory.getMessages() : params.chatHistory
        }) : this.memory;
        ...
        if (stream) {
            ...
            return (0, _utils.streamReducer)({
                stream,
                initialValue: "",
                reducer: (accumulator, part)=>accumulator += (0, _utils.extractText)(part.message.content),
