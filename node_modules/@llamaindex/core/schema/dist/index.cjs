...
var index_cjs$1 = require('../../decorator/dist/index.cjs');
var index_cjs = require('../../global/dist/index.cjs');
var index_cjs$2 = require('../../utils/dist/index.cjs');
var zod = require('zod');

function chunkSizeCheck(contentGetter, _context) {
    return function(...args) {
        const content = contentGetter.call(this, ...args);
        const chunkSize = index_cjs.Settings.chunkSize;
        const enableChunkSizeCheck = env.getEnv("ENABLE_CHUNK_SIZE_CHECK") === "true";
...
function buildNodeFromSplits(textSplits, doc, refDoc = doc, idGenerator = ()=>env.randomUUID()) {
    const nodes = [];
    const relationships = {
 ...
 */ class FileReader {
    async loadData(filePath) {
        const fileContent = await env.fs.readFile(filePath);
        const filename = env.path.basename(filePath);
    ...
    static addMetaData(filePath) {
        return (doc, index)=>{
            ...
            doc.metadata["file_path"] = env.path.resolve(filePath);
            doc.metadata["file_name"] = env.path.basename(filePath);
        };
    }
}
...
class EngineResponse {
   ...
   */ get response() {
        return index_cjs$2.extractText(this.message.content);
    }
    get delta() {
        if (!this.stream) {
...
const sentenceWindowNodeParserSchema = zod.z.object({
    windowSize: zod.z.number({
        description: "The number of sentences on each side of a sentence to capture."
    }).gt(0).default(3),
    windowMetadataKey: zod.z.string({
        description: "The metadata key to store the sentence window under."
    }).default("window"),
    originalTextMetadataKey: zod.z.string({
...
exports.EngineResponse = EngineResponse;
exports.FileReader = FileReader;
exports.ImageDocument = ImageDocument;
exports.ImageNode = ImageNode;
exports.IndexNode = IndexNode;
exports.MetadataMode = MetadataMode;
exports.ModalityType = ModalityType;
...
exports.buildNodeFromSplits = buildNodeFromSplits;
exports.jsonToNode = jsonToNode;
exports.sentenceSplitterSchema = sentenceSplitterSchema;
exports.sentenceWindowNodeParserSchema = sentenceWindowNodeParserSchema;
