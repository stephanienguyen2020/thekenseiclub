...
import { Token, OHLC, IToken, IOHLC } from '../models/NonNativeToken';
...
export class NonNativeTokenService {
    ...
    async updateTokenList(): Promise<IToken[]> {
        try {
            ...
            const listResponse = await axios.get(`${COINGECKO_API_BASE}/coins/list?include_platform=true`);
            ...
            const platformMap = new Map(tokens.map(token => [token.id, token.platforms]));

            // Process tokens in pages of 250 (max allowed per page)
            const perPage = 250;
            const totalPages = Math.ceil(tokens.length / perPage);
            console.log(`Will process ${totalPages} pages in batches of ${BATCH_SIZE}`);
            ...
            for (let page = 1; page <= totalPages; page++) {
                try {
                    ...
                    console.log(`Processing ${pageTokens.length} tokens from page ${page}`);
                        ...
                        updateOne: {
                            ...
                            update: {
                                $set: {
                                    ...
                                    market_cap_change_percentage_24h_usd: token.market_cap_change_percentage_24h || 0,
                                    updatedAt: new Date()
                                }
                            },
                            upsert: true
                    ...
                    if (operations.length > 0) {
                        ...
                        console.log(`Updated ${operations.length} tokens from page ${page}`);
                    }
                    ...
                    if (page < totalPages) {
                        if (page % BATCH_SIZE === 0) {
                            ...
                            console.log(`Completed batch of ${BATCH_SIZE} pages. Waiting ${minutes} minutes before next batch...`);
                            await delay(LONG_DELAY);
                        } else {
                            // Short delay between pages within a batch
                            console.log(`Waiting ${PAGE_DELAY/1000} seconds before next page...`);
    ...
    async getTokenList(page: number = 1, limit: number = 10, platform?: string): Promise<{
        tokens: IToken[];
        total: number;
        page: number;
        totalPages: number;
        ...
        try {
            const query = platform ? { [`platforms.${platform}`]: { $exists: true } } : {};
    ...
    async getOHLCData(coinId: string, vsCurrency: string = 'usd'): Promise<IOHLC[]> {
        return await OHLC.find({ coingeckoId: coinId, vs_currency: vsCurrency })
            .sort({ timestamp: 1 });
    }

    // Fetch and update OHLC data for all tokens
    async updateAllOHLCData(vsCurrency: string = 'usd', days: number = 365): Promise<{
        success: number;
        failed: number;
            ...
            for (let i = 0; i < tokens.length; i += batchSize) {
                ...
                for (const token of batch) {
                    try {
                        ...
                        if (axios.isAxiosError(error) && error.response?.status === 429) {
                            console.log('Rate limit hit, waiting 2 minutes before retrying...');
                ...
                if (i + batchSize < tokens.length) {
                    console.log(`Completed batch. Waiting 2 minutes before next batch...`);
                    await delay(longDelay);
                }
            }
