...
router.get('/tokens', async (req, res) => {
    try {
        ...
        const result = await nonNativeTokenService.getTokenList(page, limit, platform);
        res.json(result);
    } catch (error) {
...
router.get('/ohlc/:coinId', async (req, res) => {
    try {
        ...
        const ohlcData = await nonNativeTokenService.getOHLCData(coinId, vsCurrency);
        res.json(ohlcData);
    } catch (error) {
        res.status(500).json({ error: 'Failed to fetch OHLC data', details: error });
    }
});
...
router.post('/ohlc/update', async (req, res) => {
    try {
        ...
        const result = await nonNativeTokenService.updateAllOHLCData(vsCurrency, days);
        res.json(result);
    } catch (error) {
        res.status(500).json({ error: 'Failed to update OHLC data for all tokens', details: error });
    }
});
