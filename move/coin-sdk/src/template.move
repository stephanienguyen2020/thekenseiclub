...
public struct DAT has drop {}

fun init(witness: DAT, ctx: &mut TxContext) {
    let (treasury_cap, metadata) = coin::create_currency<DAT>(
        witness,
        9,
        b"DAT",
        b"DAT",
...
public fun update_coin_info(
    ...
    description: String,
    icon_url: vector<u8>,
    _treasury: &coin::TreasuryCap<DAT>,
    metadata: &mut coin::CoinMetadata<DAT>,
) {
...
public entry fun create_and_transfer(
    treasury_cap: &mut TreasuryCap<DAT>,
    recepient: address,
    amount: u64,
    ctx: &mut TxContext,
) {
...
public entry fun burn(treasury_cap: &mut TreasuryCap<DAT>, coin: Coin<DAT>) {
