...
public struct Proposal has store {
    ...
    total_votes: u64,
    total_points: u64,
    token_type: string::String, // Type of token used for voting (e.g., "0x123::mycoin::MYCOIN")
}
...
public entry fun close_proposal(
    ...
    // Reset vote points before recalculating
    let mut i = 0;
    let options_len = vector::length(&proposal.options);
    while (i < options_len) {
        *vector::borrow_mut(&mut proposal.vote_points, i) = 0;
        i = i + 1;
    };
    proposal.total_points = 0;

    // Recalculate votes based on provided token balances
    let voters_len = vector::length(&voters);

    // Make sure the voters and balances vectors have the same length
    assert!(voters_len == vector::length(&balances), EInvalidOption);

    let mut j = 0;
    while (j < voters_len) {
        let voter = vector::borrow(&voters, j);

        // Check if the voter has voted on this proposal
        if (vec_map::contains(&proposal.votes, voter)) {
            let option_index = *vec_map::get(&proposal.votes, voter);
            let token_balance = *vector::borrow(&balances, j);

            // Add the token balance to the option's vote points
            *vector::borrow_mut(&mut proposal.vote_points, option_index) =
                *vector::borrow(&proposal.vote_points, option_index) + token_balance;

            // Add to total points
            proposal.total_points = proposal.total_points + token_balance;
        };
